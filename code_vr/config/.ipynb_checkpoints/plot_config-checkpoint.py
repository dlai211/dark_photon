...
def getWeight(fb, sample):
    # weights for jet-faking photons
    if sample == "data24_y":
        abs_eta = abs(ak.firsts(fb['ph_eta']))
        sf = ak.full_like(abs_eta, 0.0)

        sf = ak.where((abs_eta > 0.0) & (abs_eta <= 0.6), 1.453, sf)
        sf = ak.where((abs_eta > 0.6) & (abs_eta <= 1.37), 1.603, sf)
        sf = ak.where((abs_eta > 1.37) & (abs_eta <= 1.52), 0., sf)
        sf = ak.where((abs_eta > 1.52) & (abs_eta <= 1.81), 1.942, sf)
        sf = ak.where((abs_eta > 1.81) & (abs_eta <= 2.37), 1.545, sf)
        return sf

    # weights for electron-faking photons
    if sample == "data24_eprobe":
        el_pt  = ak.fill_none(ak.firsts(fb["el_pt"]),  -1)   # MeV
        abs_eta = ak.fill_none(abs(ak.firsts(fb["el_eta"])), 0.0)
...
def getSampleDict():
    sample_dict = {
        ...
        'Zgamma': {
            ...
            'legend': r'Z($\nu\nu$)+$\gamma$',
            'tree': 'nominal',
            'filenames': ['mc23e_Zgamma_y'],
        },
        'Wgamma': {
            'color': '#ff9933',
            'legend': r'W($\ell\nu$)+$\gamma$',
            'tree': 'nominal',
            'filenames': ['mc23e_Wgamma_y'],
        },
        'gammajet_direct': {
            'color': '#000099',
            'legend': r'$\gamma$+jets direct',
            'tree': 'gammajets',
            'filenames': ['mc23e_gammajet_direct_y'],
        },
        ...
        'data_eprobe': {
            ...
            'legend': r'$e\to\gamma$',
            'tree': 'nominal',
            'filenames': ['data24_eprobe'],
        },
    }
    return sample_dict
sample_dict = getSampleDict()

def getVarDict(fb, var_name=None):
    var_dict = {}
