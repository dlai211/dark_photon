...
def plot_performance(tot2, ntuple_names, sample_dict, getVarDict, zbi, mt_val_dir, cut_name, signal_name='ggHyyd'):
    ...
    for var in var_config:
        ...
        # Ensure they are plain NumPy arrays (not Awkward-backed)
        S_counts = np.asarray(S_counts, dtype=float)
        B_counts = np.asarray(B_counts, dtype=float)
        
        # Optionally clip to avoid sqrt of negative if you have negative weights
        B_safe = np.clip(B_counts, a_min=0, a_max=None)
        SplusB_safe = np.clip(S_counts + B_counts, a_min=0, a_max=None)
        Splus1p3B_safe = np.clip(S_counts + 1.3 * B_counts, a_min=0, a_max=None)
        ...
        # Use np.divide with 'where' so invalid divides are never executed
        np.divide(
            S_counts,
            np.sqrt(B_safe),
            out=sig_simple,
        ...
        np.divide(
            S_counts,
            np.sqrt(Splus1p3B_safe),
            out=sig_s_plus_1p3b,
            where=Splus1p3B_safe > 0,
        )
...
def calculate_significance2(tot, ntuple_names, getVarDict, cut_var, cut_type, cut_values, signal_name="ggHyyd"):
...
def plot_significance(tot, ntuple_names, sample_dict, getVarDict, cut_config, sel, mt_val_dir, cut_name, signal_name='ggHyyd', other_sig=False):
    ...
    for cut_var, cut_types in cut_config.items():
        for cut_type, cut_values in cut_types.items():
            (sig_simple_list, sig_s_plus_b_list, sig_s_plus_1p3b_list, sig_binomial_list,
             sigacc_simple_list, sigacc_s_plus_b_list, sigacc_s_plus_1p3b_list, sigacc_binomial_list,
             acceptance_values) = calculate_significance2(tot, ntuple_names, getVarDict, cut_var, cut_type, cut_values)
...
def plot_n_1(tot, ntuple_names, getVarDict, zbi, getCutDict, sel, mt_val_dir, n_1_config, cut_name='n-1', signal_name="ggHyyd"):
