...
for ntuple_name in ntuple_names:
    ...
    if ntuple_name == 'data23':
        path = "/data/fpiazza/ggHyyd/Ntuples/MC23d/withVertexBDT/data23_uuy_BDT_score.root"
        ...
        path = f"/data/fpiazza/ggHyyd/Ntuples/MC23d/withVertexBDT/mc23d_{ntuple_name}_uuy_BDT_score.root"
        f = uproot.open(path)['nominal']
    ...
    fb = fb[fb['n_mu'] == 2]
    log_step(ntuple_name, step, "n_mu==2", fb, start_time); step += 1
    ...
    fb = fb[mt_tmp >= 100]
    fb = fb[mt_tmp <= 140]
    log_step(ntuple_name, step, "140GeV>=mT>=100GeV", fb, start_time); step += 1
